...
namespace HidSharp.Platform.SystemEvents
{
    ...
    {
        public static readonly IntPtr IntPtrNegativeOne = (IntPtr.Size == 8) ? new IntPtr((long)(-1)) : new IntPtr(-1);
    ...
    unsafe class LinuxNativeMethods : PosixNativeMethods
    {
        ...
        {
            if (clock_gettime(CLOCK_MONOTONIC, out timespec timespec) < 0) { throw new InvalidOperationException(); }
    ...
    unsafe class MacOSNativeMethods : PosixNativeMethods
    {
        ...
        static MacOSNativeMethods()
        {
            ...
            mach_timebase_info(out mach_timebase_info_data_t info);
            _scale = info.numer / (double)info.denom / 1e6;
        }
    ...
    internal abstract class PosixEventManager : EventManager
    {
        ...
        {
            if (_jobs.TryGetValue(jobObject, out Action job))
            {
                job();
            }
        }
            ...
            while (true)
            {
                ...
                while (offset <= bytes - inotifySize)
                {
                    ...
                    if (0 != (ev.mask & LinuxNativeMethods.IN_ATTRIB))
                    {
                        lock (SyncRoot)
                        {
                            if (_watchDescriptors.TryGetValue(ev.wd, out JobHandle jobHandle))
                            {
    ...
    internal sealed class MacOSEventManager : PosixEventManager
    {
            ...
            if (shmName != null)
            {
                ...
                if (!hasThread)
                {
                    if (MacOSNativeMethods.KERN_SUCCESS !=
                        MacOSNativeMethods.mach_port_allocate(MacOSNativeMethods.GetMachTaskSelf(), MacOSNativeMethods.MACH_PORT_RIGHT_RECEIVE, out _notifyFD))
                    {
                ...
                var ret = MacOSNativeMethods.notify_register_mach_port(shmName, ref _notifyFD, MacOSNativeMethods.NOTIFY_REUSE, out int token);
        ...
        unsafe void RunNotifyThread()
        {
            ...
            while (true)
            {
                ...
                lock (SyncRoot)
                {
                    if (_notifyTokens.TryGetValue(token, out JobHandle jobHandle))
                    {
                        RunJobObject(jobHandle);
                    }
                }
