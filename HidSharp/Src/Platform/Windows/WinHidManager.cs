...
namespace HidSharp.Platform.Windows
{
    sealed class WinHidManager : HidManager
    {
        ...
        protected override void Run(Action readyCallback)
        {
            ...
            while (true)
            {
                int result = NativeMethods.GetMessage(out NativeMethods.MSG msg, hwnd, 0, 0);
        ...
        static unsafe void SerialWatcherThread()
        {
            ...
            try
            {
                if (0 == NativeMethods.RegOpenKeyEx(new IntPtr(unchecked((int)NativeMethods.HKEY_LOCAL_MACHINE)), @"HARDWARE\DEVICEMAP\SERIALCOMM", 0, NativeMethods.KEY_NOTIFY, out IntPtr handle))
                {
                    try
                    {
        ...
        protected override object[] GetBleDeviceKeys()
        {
            ...
            if (_bleIsSupported)
            {
                NativeMethods.EnumerateDeviceInterfaces(NativeMethods.GuidForBluetoothLEDevice, (deviceInfoSet, deviceInfoData, deviceInterfaceData, deviceID, devicePath) =>
                    {
                        if (NativeMethods.TryGetDeviceRegistryProperty(deviceInfoSet, ref deviceInfoData, NativeMethods.SPDRP_FRIENDLYNAME, out string friendlyName))
                        {
