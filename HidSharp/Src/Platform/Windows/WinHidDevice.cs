    ...
    sealed partial class WinHidDevice : HidDevice
    {
                ...
                if (!TryOpenToGetInfo(handle =>
                    {
                        ...
                        {
                            if (!NativeMethods.HidD_GetPreparsedData(handle, out IntPtr preparsed)) { return false; }

                            try
                            {
                                int statusCaps = NativeMethods.HidP_GetCaps(preparsed, out NativeMethods.HIDP_CAPS caps);
        ...
        */
        ...
        bool TryGetDeviceUsbRoot(out uint devInst)
        {
            if (0 == NativeMethods.CM_Locate_DevNode(out devInst, _id))
            {
                ...
                while (true)
                {
                    if (0 != NativeMethods.CM_Get_Parent(out uint parentDevInst, devInst)) { break; }

                    if (0 != NativeMethods.CM_Get_Device_ID(parentDevInst, out string parentDeviceID)) { break; }
        ...
        */
        ...
        public override string[] GetSerialPorts()
        {
            ...
            if (TryGetDeviceUsbRoot(out uint devInst))
            {
                ...
                if (0 == NativeMethods.CM_Get_Child(out devInst, devInst))
                {
                    do
                    {
                        if (0 == NativeMethods.CM_Get_Device_ID(devInst, out string deviceID))
                        {
                            NativeMethods.EnumerateDeviceInterfaces(NativeMethods.GuidForComPort, deviceID, (deviceInfoSet, deviceInfoData, _, __, devicePath) =>
                                {
                                    if (NativeMethods.TryGetSerialPortFriendlyName(deviceInfoSet, ref deviceInfoData, out string friendlyName) &&
                                        NativeMethods.TryGetSerialPortName(deviceInfoSet, ref deviceInfoData, out string portName))
                                    {
                                        ports.Add(@"\\.\" + portName);
                                    }
                                });
