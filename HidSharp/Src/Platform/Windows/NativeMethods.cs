...
namespace HidSharp.Platform.Windows
{
    unsafe static class NativeMethods
    {
        ...
        public static int CM_Get_Device_ID(uint devInst, out string deviceID)
        {
            ...
            ret = CM_Get_Device_ID_Size(out int length, devInst);
            if (ret != 0) { return ret; }
        ...
        {
            HidD_GetHidGuid(out Guid guid); return guid;
        }
            ...
            out string devicePath)
        {
            ...
            if (NativeMethods.SetupDiGetDeviceInterfaceDetail(deviceInfoSet, ref deviceInterfaceData, out SP_DEVICE_INTERFACE_DETAIL_DATA didetail))
            {
        ...
        public static bool TryGetDeviceRegistryProperty(HDEVINFO deviceInfoSet, ref SP_DEVINFO_DATA deviceInfoData, uint property, out string value)
        {
            ...
            char[] propertyValueChars = new char[64]; int propertyValueLength = 63 * 2;
            if (SetupDiGetDeviceRegistryProperty(deviceInfoSet, ref deviceInfoData, property, out uint propertyDataType,
            ...
            if (hkey != (IntPtr)(-1))
            {
                try
                {
                    ...
                    if (0 == NativeMethods.RegQueryValueEx(hkey, "PortName", 0, IntPtr.Zero, portNameChars, ref portNameLength))
                    {
                        ...
                        {
                            if (int.TryParse(newPortName.Substring(3), NumberStyles.Integer, CultureInfo.InvariantCulture, out int newPortNumber) &&
        ...
        public static void EnumerateDeviceInterfaces(Guid guid, string deviceIDToFilterTo, EnumerateDeviceInterfacesCallback callback)
        {
            EnumerateDevicesCore(NativeMethods.SetupDiGetClassDevs(guid, deviceIDToFilterTo, IntPtr.Zero, NativeMethods.DIGCF.DeviceInterface | NativeMethods.DIGCF.Present),
                (devInfo, dvi, deviceID) =>
                {
                    ...
                    {
                        if (NativeMethods.SetupDiGetDeviceInterfaceDevicePath(devInfo, ref did, out string devicePath))
                        {
        ...
        static void EnumerateDevicesCore(NativeMethods.HDEVINFO devInfo, EnumerateDevicesCallback callback)
        {
            if (devInfo.IsValid)
            {
                try
                {
                    ...
                    {
                        if (0 != NativeMethods.CM_Get_Device_ID(dvi.DevInst, out string deviceID)) { continue; }
        ...
        public static NativeMethods.BTH_LE_GATT_SERVICE[] BluetoothGATTGetServices(IntPtr handle)
        {
            ...
            error = NativeMethods.BluetoothGATTGetServices(handle, 0, null, out ushort allocated);
            if (error != NativeMethods.ERROR_MORE_DATA) { return null; }

            var services = new NativeMethods.BTH_LE_GATT_SERVICE[allocated];
            if (allocated > 0)
            {
                error = NativeMethods.BluetoothGATTGetServices(handle, allocated, services, out ushort returned);
                                                                                   ...
                                                                                   ref BTH_LE_GATT_SERVICE service)
        {
            ...
            error = NativeMethods.BluetoothGATTGetCharacteristics(handle, ref service, 0, null, out ushort allocated);
            ...
            if (allocated > 0)
            {
                error = NativeMethods.BluetoothGATTGetCharacteristics(handle, ref service, allocated, characteristics, out ushort returned);
                if (error != 0) { return null; }
                if (allocated != returned) { return null; }
            }
                                                                           ...
                                                                           ref BTH_LE_GATT_CHARACTERISTIC characteristic)
        {
            ...
            error = NativeMethods.BluetoothGATTGetDescriptors(handle, ref characteristic, 0, null, out ushort allocated);
            if (error == NativeMethods.ERROR_NOT_FOUND) { allocated = 0; }
            else
            if (error != NativeMethods.ERROR_MORE_DATA) { return null; }
            ...
            if (allocated > 0)
            {
                error = NativeMethods.BluetoothGATTGetDescriptors(handle, ref characteristic, allocated, descriptors, out ushort returned);
                if (error != 0) { return null; }
                if (allocated != returned) { return null; }
            }
